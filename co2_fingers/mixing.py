...
def _load_concentration_field(
    ...
    crop : dict
        ...
        Pass _FULL_IMAGE_CROP when using manual roi only.
    invert : bool
        If True, return 1 - intensity (for dyes that darken with concentration).
    roi : tuple or None
        (x0, y0, x1, y1) secondary crop applied in post-crop pixel coordinates.
...
def sherwood_number(
    ...
    roi : tuple or None
        (x0, y0, x1, y1) sub-region in post-crop pixel coordinates.
    ...
    for i, path in enumerate(image_paths):
        try:
            c = _load_concentration_field(str(path), crop, invert=invert, roi=roi)
            c_bar[i] = c.mean()
...
# Sentinel crop that passes the full image through _load_concentration_field.
# Use with crop_override=_FULL_IMAGE_CROP when relying solely on roi.
_FULL_IMAGE_CROP = {'y_top': 0, 'y_bot': None, 'x_left': 0, 'x_right': None}
...
def run_sherwood(
    ...
    roi : tuple or None
        (x0, y0, x1, y1) manual sub-region in post-crop pixel coordinates.
        Applied after crop / crop_override.  Use the ROI visualisation cell
        in Colab to determine coordinates before running.
    crop_override : dict or None
        Manually supplied crop dict (y_top, y_bot, x_left, x_right) that
        replaces the config crop entirely.  Pass _FULL_IMAGE_CROP to bypass
        the config crop and rely solely on roi.
    ...
    Examples
    --------
    # Default: use config crop only
    sh = cf.run_sherwood(config_path=..., csv_path=..., image_dir=..., H_m=0.27)

    # Sub-region within the config crop
    sh = cf.run_sherwood(..., roi=(100, 50, 900, 600))

    # Replace config crop entirely
    sh = cf.run_sherwood(..., crop_override=dict(y_top=50, y_bot=600,
                                                  x_left=100, x_right=900))

    # Bypass config crop, use only roi
    from co2_fingers.mixing import _FULL_IMAGE_CROP
    sh = cf.run_sherwood(..., crop_override=_FULL_IMAGE_CROP,
                              roi=(100, 50, 900, 600))
    """
    ...
    if roi is not None:
        print(f"  ROI active: x=[{roi[0]}:{roi[2]}], y=[{roi[1]}:{roi[3]}]  "
...
def run_horizontal_slicing(
    ...
    roi : tuple or None
        (x0, y0, x1, y1) manual sub-region in post-crop pixel coordinates.
    crop_override : dict or None
        Replaces the config crop entirely.
